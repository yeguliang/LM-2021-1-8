...
user.post('/api/register', asyncHandler(async (req, res) => {
        const {
            body: {
                ...
                code = "",
                is_hidden = false,
                create_at = moment(new Date()).format(),
                updata_at = moment(new Date()).format()
            }
        } = req
        if (!name || !psd || !code) {
            res.send('参数错误!')
    ...
    user.put('/api/reset', asyncHandler(async (req, res) => {
        ...
        let user_list = await queryList("user")
        let userInfo = await _.find(user_list, {
            name: name
        })
        if (!userInfo) {
